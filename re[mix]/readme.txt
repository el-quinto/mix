...
————————————————————
midi in/out- re:mix recieves all press data and outputs it as midi if midi out is enabled. you can also send midi to the device to trigger press data. since midi only has 128 values, you need to use the additional midi(row9-16).amxd for the bottom 8 rows of a 256.
piano roll names.adg - placing this on a track makes it so u can see the row/button names on the piano roll.
————————————————————
